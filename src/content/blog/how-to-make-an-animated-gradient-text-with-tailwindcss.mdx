...
tags: ['tailwindcss','css','vuejs']
coverImage:
  src: '/images/react-tailwind-cover.jpg'
  alt: 'The logo of React and Tailwind CSS'
---
...
<template>
  <span
    :class="`bg-gradient-to-r ${props.from} ${props.via} ${props.to} animate-gradient bg-300% bg-clip-text text-transparent`">
    {{ props.text }}
  </span>
</template>
```
